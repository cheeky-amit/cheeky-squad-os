...
2. **If set to `1`:**
   - Invoke `${CLAUDE_PLUGIN_ROOT}/scripts/spawn.sh` with the squad mode and roster path. The script launches each teammate's Claude session with `--worktree <role-name>` (worktrees doc — CLI flag mechanism, not frontmatter propagation).
   - Construct each teammate's spawn prompt using the template above and pass it as the initial prompt to the teammate.
...
3. **If unset, `0`, or anything else:**
   ...
   - If user declines: fall back to **One-time mode dispatch** (sequential subagents). Print a warning: *"Falling back to subagents — mailbox, shared task list, and direct teammate-to-teammate messaging are unavailable. Per-teammate worktree isolation is replaced by per-subagent worktree (via `isolation: worktree` frontmatter where set)."*
